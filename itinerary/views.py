...
class GenerateItineraryView(APIView):
    ...
    def post(self, request):
        ...
        if travelers < 1 or travelers > 20:
            return Response({"error": "travelers must be between 1 and 20"}, status=400)
...
class RegenerateDayView(APIView):
    ...
    def post(self, request, pk):
        ...
        try:
            ...
            return Response({"error": "day_number must be an integer"}, status=400)
        ...
        try:
            ...
            new_day = regenerate_day(
                trip_dict,
                day_number,
                old_day,
                all_days,
                blocked_regions=adjacent_regions,
                max_retries=2,
            )
            ...
            # 9. Update itinerary JSON + recalculate ALL summary totals
            updated_days = [
                new_day.model_dump() if d.get("day_number") == day_number else d
                for d in all_days
            ]
            raw["days"] = updated_days

            summary = raw["summary"]
            summary["grand_total_inr"] = sum(
                d["day_total_cost_inr"] for d in updated_days
            )
            summary["accommodation_total_inr"] = sum(
                d["accommodation_cost_inr"] for d in updated_days
            )
            summary["local_transport_total_inr"] = sum(
                d["local_transport_cost_inr"] for d in updated_days
            )
            summary["activities_total_inr"] = sum(
                sum(a["cost_inr"] for a in d.get("activities", []))
                for d in updated_days
            )
            summary["food_total_inr"] = sum(
                d["day_total_cost_inr"]
                - d["accommodation_cost_inr"]
                - d["local_transport_cost_inr"]
                - sum(a["cost_inr"] for a in d.get("activities", []))
                for d in updated_days
            )
            raw["summary"] = summary
