    ...
    <>
      ...
      <motion.div
        ...
        style={{
          ...
          display: 'flex', flexDirection: 'column',
          alignItems: 'center', justifyContent: 'center',
          padding: 'clamp(40px, 8vw, 80px) 16px', position: 'relative', overflow: 'hidden',
      ...
      >
        ...
        <div style={{
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginTop: 24 }}>
                    ...
                    {days > 0 && days <= 14 && (
                      <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} style={{
                        marginTop: 8, fontSize: 15, color: 'rgba(255,255,255,0.6)', fontFamily: "'Inter', sans-serif"  // ✅ 13→15
                      }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 24 }}>
                    {[
                      ...
                      <motion.div key={b.id} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }} onClick={() => update('budget_tier', b.id)} style={{
                        ...
                        <div style={{
                          fontFamily: "'Inter', sans-serif", fontSize: 17, fontWeight: 600,  // ✅ 15→17
                          color: data.budget_tier === b.id ? '#fff' : 'rgba(255,255,255,0.8)',
                        }}>{b.label}</div>
                        <div style={{
                          fontFamily: "'Inter', sans-serif", fontSize: 15, color: 'rgba(255,255,255,0.45)', marginTop: 4,  // ✅ 13→15
                        }}>{b.desc}</div>
                      </motion.div>
                    ))}
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 24 }}>
                    {[
                      ...
                      <motion.div key={v.id} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => update('vibe', v.id)} style={{
                        ...
                        <div style={{
                          fontFamily: "'Inter', sans-serif", fontSize: 16, fontWeight: 600,  // ✅ 14→16
                    ...
                    <div style={{ maxHeight: 180, overflowY: 'auto', display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 16 }}>
                      {filteredCities.slice(0, 15).map(city => (
                        <span key={city} onClick={() => { update('origin', city); setCitySearch(city) }} style={{
                          padding: '8px 16px', borderRadius: 999, fontFamily: "'Inter', sans-serif", fontSize: 15, fontWeight: 500,  // ✅ 13→15, padding up too
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginTop: 24 }}>
                    {[
                      ...
                      <motion.div key={m.id} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => update('meal_pref', m.id)} style={{
                        ...
                        <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 17, fontWeight: 600, color: data.meal_pref === m.id ? '#fff' : 'rgba(255,255,255,0.8)' }}>{m.label}</div>  {/* ✅ 15→17 */}
                        <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 15, color: 'rgba(255,255,255,0.45)', marginTop: 4 }}>{m.desc}</div>  {/* ✅ 13→15 */}
                      </motion.div>
                    ))}
                  </div>
                </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 24 }}>
                    {[
                      ...
                      <motion.div key={g.id} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => update('group_type', g.id)} style={{
                        ...
                        <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 17, fontWeight: 600, color: data.group_type === g.id ? '#fff' : 'rgba(255,255,255,0.8)' }}>{g.label}</div>  {/* ✅ 15→17 */}
                      </motion.div>
                    ))}
                  </div>
                </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 36 }}>
            <motion.button
              whileHover={step > 1 ? { backgroundColor: 'rgba(255,255,255,0.1)' } : {}}
              onClick={() => step > 1 && paginate(step - 1)}
              style={{
                display: 'flex', alignItems: 'center', gap: 6,
                padding: '12px 20px', borderRadius: 999,
                background: 'transparent', border: '1px solid rgba(255,255,255,0.2)',
                color: 'rgba(255,255,255,0.9)', fontFamily: "'Inter', sans-serif", fontSize: 15, fontWeight: 500,
                cursor: step === 1 ? 'not-allowed' : 'pointer', opacity: step === 1 ? 0 : 1, transition: 'all 0.2s'
              }}>
...
const labelSt = {
  fontFamily: "'Inter', sans-serif", fontSize: 14, fontWeight: 500,  // ✅ 12→14
